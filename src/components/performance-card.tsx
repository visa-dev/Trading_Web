...
export function PerformanceCard({ post }: PerformanceCardProps) {
  ...
  const getPerformanceBg = (winRate: number) => {
      ...
      <Card className="card-material h-full overflow-hidden hover:border-yellow-400/50 transition-all duration-500">
                ...
                {post.title}
                  ...
                  <div className="flex items-center justify-between text-sm text-muted-foreground">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className={`flex items-center space-x-2 font-medium ${getPerformanceColor(post.winRate)}`}>
                        <Target className="w-4 h-4" />
                        <span>{formatPercentage(post.winRate)} Win Rate</span>
                      </div>
                    </div>
        ...
        <CardContent className="space-y-6 flex-1 flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-4">
            <motion.div 
              className={`bg-gradient-to-br ${getPerformanceBg(post.winRate)} p-4 rounded-xl border border-blue-500/20 hover:border-blue-400/50 transition-all duration-300`}
              whileHover={{ scale: 1.02, y: -2 }}
            >
              <div className="flex items-center space-x-3">
                <div className={`w-10 h-10 bg-gradient-to-r ${getPerformanceBg(post.winRate)} rounded-xl flex items-center justify-center`}>
                  <Target className="w-5 h-5 text-blue-400" />
                </div>
                <div>
                  <div className="text-xs font-medium text-blue-400 uppercase tracking-wide">Win Rate</div>
                  <div className="text-lg font-bold text-blue-300">{formatPercentage(post.winRate)}</div>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div className="text-lg font-bold text-yellow-300">{post.riskReward.toFixed(2)}</div>
                </div>
              </div>
            </motion.div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div className="text-lg font-bold text-red-300">{formatPercentage(post.drawdown)}</div>
                </div>
              </div>
              <motion.div 
